...
func _ready():
	...
	PlayerInWorld = get_tree().get_first_node_in_group("Player")
	#GlobalScript.game_state.the_enemy = self

func _process(delta):
	Arm.look_at(PlayerInWorld.position)
...
func dummy_input():
	...
	if is_instance_valid(PlayerInWorld):
		if position.distance_to(PlayerInWorld.position) >= 300:
			if global_position.direction_to(PlayerInWorld.global_position).x < 0:
				movement_input.x -= 1
			elif global_position.direction_to(PlayerInWorld.global_position).x > 0:
				movement_input.x += 1
			if global_position.direction_to(PlayerInWorld.global_position).y < 0:
				movement_input.y -= 1
			elif global_position.direction_to(PlayerInWorld.global_position).y > 0:
				movement_input.y += 1

		if $RandomRangeTimer.is_stopped():
			jump_input = true
			$RandomRangeTimer.start()
		else:
			jump_input = false
